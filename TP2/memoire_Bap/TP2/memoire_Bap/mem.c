...
ez, (int) ez->size, ez->next, ez->prev, ez->nxfr, ez->free ? "vrai" : "faux");
}
...
void* mem_alloc(size_t size) {
    ...
    entete_zone_occupee *ezo_temp // Variable utilisé pour update tous les nxfr
    = ezo_P ;
    while (ezo_temp->prev){
          ezo_temp->prev->nxfr = new_ezl_P ;
          ezo_temp = ezo_temp->prev;
    }
...
void mem_show(void (*print)(void *adresse, size_t size, int free)) {
    ...
    while (ezc) {
        ...
        printf("Zprec : %p, Zsuiv : %p, PzLib : %p \n", ezc->prev, ezc->next, ezc->nxfr);
        ezc = ezc->next ;
    }
}
